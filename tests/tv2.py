...
F5TTS has the same problem and is deliberately not fixed the same way -- see
WITHHELD below. Its dependency resolves cleanly; it just costs more than the
other four together.
...
WITHHELD = {
    ...
    "f5-tts": (
        ...
        "google-auth, sentry-sdk, pyarrow), and it drags rich 15.0.0 down to "
        "13.9.4 through cached-path -- more cost than the other four "
        "promotions together. F5TTS stays behind its extra"
    ),
}
...
def test_emptied_extras_survive_as_aliases():
    ...
    for name in ("supertonic", "hunyuan3d"):
...
def test_f5_tts_extra_still_installs_something():
    """It was not promoted, so emptying it would strand the node entirely."""
    extras = _project()["optional-dependencies"]
    assert extras["f5-tts"], (
        "f5-tts is not a base dependency, so its extra must still install the "
        "package -- an empty alias here would leave F5TTS with no install path"
    )
    assert {_normalise(s) for s in extras["f5-tts"]} == {"f5-tts"}, (
        "torchcodec is a base dependency already; repeating its `==` pin here "
        "is a second place to update and a conflict waiting to happen"
    )
