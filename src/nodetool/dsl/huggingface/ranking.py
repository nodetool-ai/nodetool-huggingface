...
class Reranker(GraphNode):
    ...
    model: types.HFReranker | GraphNode | tuple[GraphNode, str] = Field(default=types.HFReranker(type='hf.reranker', repo_id='', path=None, allow_patterns=None, ignore_patterns=None), description='The model ID to use for reranking')
    query: str | GraphNode | tuple[GraphNode, str] = Field(default='', description='The query text to compare against candidates')
    candidates: list[str] | GraphNode | tuple[GraphNode, str] = Field(default=[], description='List of candidate texts to rank')
